...
def draw_board(board):
	for c in range(COLUMN_COUNT):
		for r in range(ROW_COUNT):
			...
			pygame.draw.circle(screen, LIGHT, (int(c*SQUARESIZE+SQUARESIZE/2), int(r*SQUARESIZE+SQUARESIZE+SQUARESIZE/2)), RADIUS)
			...
			if turn == 0:
				...
				if is_valid_location(board, col):
					...
					if winning_move(board, 1):
						label = myfont.render("Player Red wins!!", 1, WHITE)
			...
			# # Ask for Player 2 Input
				...
				if is_valid_location(board, col):
					...
					if winning_move(board, 2):
						label = myfont.render("Player Blue wins!!", 1, WHITE)
						screen.blit(label, (40,10))
						game_over = True
